...
{
    public static List<T> FromCSVText_List<T>(string strCSV, Action<string> OnError)
        => FromCSVText_List<T>(Encoding.ASCII.GetBytes(strCSV), OnError);
...
namespace Sinbad
{
    ...
    public static class CsvUtil
    {
        ...
        // @param strict If true, log errors if a line doesn't have enough
        //   fields as per the header. If false, ignores and just fills what it can
        public static List<T> LoadObjects<T>(TextReader rdr, Action<string> OnError) 
            // where T : new()
        {
            var ret = new List<T>();
        ...
        public static void LoadObject<T>(TextReader rdr, ref T destObject, Action<string> OnError)
        {
            ...
            while ((line = rdr.ReadLine()) != null)
            {
                ...
                {
                    OnError?.Invoke($"CsvUtil: ignoring line '{line}': not enough fields");
                }
            }
        ...
        private static IEnumerable<string> EnumerateCsvLine(string line)
        {
            ...
            {
                if (m.Groups[1].Value == ",") // csv에서 공란인 경우
                    yield return " ,";
